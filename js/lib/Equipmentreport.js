...
var target = new Vue({
    ...
    data: function data() {
        ...
        var depositEndDataFun = function depositEndDataFun(rule, value, callback) {
            if (value) {
                _this.searchData.startData = basis.baseData(_this.searchData.startData, 1);
                _this.searchData.endData = basis.baseData(_this.searchData.endData, 1);
                var startData = _this.searchData.startData.split("-");
                var ST = new Date(startData[0], startData[1], startData[2]);
                var endData = _this.searchData.endData.split("-");
                var ET = new Date(endData[0], endData[1], endData[2]);
        ...
        return {
            ...
            searchverification: { // 验证
                startData: [{ required: true, message: '必填' }, {
                    validator: depositEndDataFun
                }],
                endData: [{ required: true, message: '必填' }, {
                    validator: depositEndDataFun
                }]
