        ...
        return h('div', [h('i-Button', {
            ...
            on: {
                click: function click() {
                    ...
                    //获取单个当前检验项信息
                    axios.get(webApiUrl + '/api/NonconformingFlow/GetCurrentCheckInfo?ProcessGuid=' + params.row.processId + '&NonconformingFlowId=' + NonconformingFlowId).then(function (response) {
                    ...
                    axios.get(webApiUrl + '/api/NonconformingFlow/GetAuditInfoByNFID?NFID=' + NonconformingFlowId + "&auditType=" + 0).then(function (response) {
                        if (response.data.status) {
                    ...
                    axios.get(webApiUrl + '/api/NonconformingFlow/GetAuditInfoByNFID?NFID=' + NonconformingFlowId + "&auditType=" + 1).then(function (response) {
                        if (response.data.status) {
                            var data = response.data.data;
...
var checktitleRows = [[{ fields: ['processName'], title: '工序名称', titleAlign: 'center', rowspan: 2 }, { fields: ['qualityName'], title: '检测项目', titleAlign: 'center', rowspan: 2 }, { fields: ['evaluation'], title: '评价测量技术', titleAlign: 'center', rowspan: 2 }, { fields: ['myCheck', 'professionalCheck'], title: '实测数据', titleAlign: 'center', colspan: 2 }, { fields: ['standard'], title: '检测标准', titleAlign: 'center', rowspan: 2 }, { fields: ['cmmFileId'], title: '三坐标文件', titleAlign: 'center', rowspan: 2 }], [{ fields: ['myCheck'], title: '自检', titleAlign: 'center' }, { fields: ['professionalCheck'], title: '专检', titleAlign: 'center' }]];
...
var target = new Vue({
    ...
    methods: {
        ...
        getProcess: function getProcess() {
            var _ = this;
            axios.get(webApiUrl + '/api/ProductBasicInfo/GetProcessBySkuId?random=' + Math.random()).then(function (response) {
                if (response.data.status) {
        ...
        GetBatchNo: function GetBatchNo() {
            var _ = this;
            axios.get(webApiUrl + '/api/NonconformingFlow/GetSelectBatchNo?random=' + Math.random()).then(function (response) {
                if (response.data.status) {
        ...
        GetSelectUser: function GetSelectUser(type) {
            var _ = this;
            axios.get(webApiUrl + '/api/NonconformingFlow/GetUsers?type=' + type).then(function (response) {
                if (response.data.status) {
                    if (type == 1) {
        ...
        showAllEditClick: function showAllEditClick(ev) {
            var oEvent = ev || event;
            oEvent.cancelBubble = true;
            oEvent.stopPropagation();
            var _ = this;
            //获取当前工序前检验项信息
            axios.get(webApiUrl + '/api/NonconformingFlow/GetAllCheckInfo?NonconformingFlowId=' + NonconformingFlowId + "&ProcessGuid=" + processId).then(function (response) {
                if (response.data.status) {
                    CheckConvertData(response.data.data);
        ...
        changePage: function changePage(pageIndex) {
            ...
            debugger;
            axios.post(webApiUrl + '/api/NonconformingFlow/GetPageLists', _.searchModel).then(function (response) {
                if (response.data.status) {
        ...
        save: function save() {
            var _ = this;
            _.$refs[_.state + 'EditForm'].validate(function (valid) {
                if (valid) {
        ...
        cellMerge: function cellMerge(rowIndex, rowData, field) {
