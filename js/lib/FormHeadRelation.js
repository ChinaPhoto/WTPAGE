...
var target = new Vue({
    ...
    data: function data() {
        return {
            uploadUrl: webApiUrl + '/api/FormHeadRelation/upload?random=' + Math.random(), // 上传路径
            total: 0, //总条数
            baseModel: {
                formHeadId: 0,
                formId: 0,
                    ...
                    return h('div', [h('i-Button', {
                        ...
                        on: {
                            click: function click() {
                                var _ = target;
                                _.baseModel.formHeadId = params.row.formHeadId;
    ...
    methods: {
        ...
        download: function download() {
            ...
            axios.post(webApiUrl + '/api/FormHeadRelation/Download?random=' + Math.random(), _.searchModel).then(function (response) {
        ...
        changePage: function changePage(pageIndex) {
            ...
            axios.post(webApiUrl + '/api/FormHeadRelation/GetPageList?random=' + Math.random(), _.searchModel).then(function (response) {
                if (response.data.status) {
        ...
        del: function del() {
            var _ = this;
            this.loading = true;
            axios.post(webApiUrl + "/api/FormHeadRelation/delete?random=" + Math.random(), {
                id: _.baseModel.id
            }).then(function (res) {
                if (res.data.status) {
        ...
        save: function save() {
            ...
            this.$refs['editForm'].validate(function (valid) {
                if (valid) {
                    _this.loading = true;
                    var url = webApiUrl + "/api/FormHeadRelation/insert?random=" + Math.random();
                    if (_.baseModel.id != 0) {
                        url = webApiUrl + "/api/FormHeadRelation/update?random=" + Math.random();
                    }
